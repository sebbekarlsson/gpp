...
AST_T* json_visit(json_ast_T* jnode)
{
  switch (jnode->type)
  {
    case JSON_AST_KEY_VALUE_LIST: return json_visit_key_value_list(jnode); break;
    case JSON_AST_KEY_VALUE: return json_visit_key_value(jnode); break;
    case JSON_AST_STRING: return json_visit_string(jnode); break;
    case JSON_AST_INTEGER: return json_visit_integer(jnode); break;
    case JSON_AST_FLOAT: return json_visit_float(jnode); break;
    case JSON_AST_LIST: return json_visit_list(jnode); break;
    default: { printf("[json_visit]: Unhandled node.\n"); exit(1); } break;
  }
}
...
AST_T* json_visit_key_value_list(json_ast_T* jnode)
{
  ...
  for (int i = 0; i < jnode->key_value_list_size; i++)
  {
    ...
    {
      ast->object_vars = realloc(ast->object_vars, sizeof(struct AST_STRUCT*) * ast->object_vars_size);
