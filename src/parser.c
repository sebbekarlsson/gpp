...
AST_T *parser_parse(parser_T *parser, AST_T *parent) {
  ...
  while (parser->token->type != TOKEN_EOF) {
    ...
    if (ast->root_items == (void *)0) {
      ...
      ast->root_items = realloc(ast->root_items, sizeof(struct AST_STRUCT *) *
                                                     ast->root_items_size);
...
AST_T *parser_parse_expr(parser_T *parser, AST_T *parent) {
  ...
  case TOKEN_RAW:
    ast = parser_parse_raw(parser, parent);
    break;
  case TOKEN_COMP_BEGIN:
    ast = parser_parse_comp(parser, parent);
    break;
  case TOKEN_ID:
    ast = parser_parse_id(parser, parent);
    break;
  case TOKEN_STRING:
    ast = parser_parse_string(parser, parent);
    break;
  case TOKEN_NUMBER:
    ast = parser_parse_number(parser, parent);
    break;
  case TOKEN_LPAREN:
    ast = parser_parse_group(parser, parent);
    break;
  case TOKEN_LBRACKET:
    ast = parser_parse_group(parser, parent);
    break;
  case TOKEN_COMMENT:
    ast = parser_parse_comment(parser, parent);
    break;
  default:
    if (parser->token->type != TOKEN_EOF && parser->token->type != TOKEN_DOT) {
      printf("[Parser]: Unexpected `%s` `%s`\n", token_to_str(parser->token),
             parser->token->value);
      exit(1);
    }
    break;
...
AST_T *parser_parse_comp(parser_T *parser, AST_T *parent) {
  ...
  while (parser->token->type != TOKEN_COMP_END &&
    ...
    if (ast->group_items == (void *)0) {
      ast->group_items =
          calloc(ast->group_items_size, sizeof(struct AST_STRUCT *));
...
AST_T *parser_parse_id(parser_T *parser, AST_T *parent) {
  ...
  char *var_name =
      (char *)calloc(strlen(parser->prev_token->value) + 1, sizeof(char));
  strcpy(var_name, parser->prev_token->value);

  if (parser->token->type == TOKEN_LBRACKET ||
      parser->token->type == TOKEN_LPAREN) {
